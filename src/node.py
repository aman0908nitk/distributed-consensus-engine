...
class Node:
    def __init__(self, node_id: str, ip_map: Dict[str, str], listen_port: int = 5000):
        ...
        self.mode = os.getenv("CONSENSUS_MODE", "A")                
    ...
    async def start(self):
        ...
        logging.info(f"Initialized node server on port {self.listen_port}. System Mode: {self.mode}")
    ...
    async def election_timeout_loop(self):
        try:
            ...
            if self.role != "leader":
                logging.info("Heartbeat window missed. Transitioning to Candidate.")
    ...
    async def start_election(self):
        ...
        self.election_timeout = random.uniform(1.5, 3.0)
        logging.info(f"ELECTION_TRIGGERED: Begun term {self.current_term}")
    ...
    async def handle_connection(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        try:
            ...
            logging.error(f"Error while parsing message context: {e}")
        finally:
            writer.close()
    ...
    async def process_message(self, msg: dict):     
        ...
        if msg_type == "MODE_CHANGE":
            ...
            logging.info(f"Mode dynamically updated to: {self.mode}")
            return
        if msg_type == "TX_SUBMIT":
            ...
            if self.role == "leader":
                ...
                logging.warning(f"Rejecting submission of tx '{tx_val}' on non-leader node.")
            return
        ...
        if msg_type == "VOTE_REQUEST":
            ...
            if (self.voted_for is None or self.voted_for == candidate) and term == self.current_term:         
                ...
                logging.info(f"Vote cast positively for candidate: {candidate} in term {term}")
                vote_reply = {
                    "type": "VOTE_RESPONSE",
                    "term": self.current_term,             
            ...
            if term >= self.current_term:              
                ...
                if self.role != "follower":
                    logging.info(f"Stepping down from candidate/leader. Authoritative leader detected: {leader}")
                    self.role = "follower"
                self.reset_election_timer()
        elif self.mode == "A":
    ...
    async def propose_paxos_transaction(self, tx: str):
        ...
        proposal_id = self.current_term * 10000 + self.proposal_counter
        logging.info(f"Paxos Prepare phase triggered with Proposal ID: {proposal_id}")     
        prepare_msg = {
            "type": "PAXOS_PREPARE",
    ...
    async def process_paxos_message(self, msg: dict):   
        ...
        if msg_type == "PAXOS_PREPARE":
            ...
            if self.role == "leader" and prop_id in self.paxos_promises:
                if any(p["node_id"] == voter for p in self.paxos_promises[prop_id]):
                    return
                self.paxos_promises[prop_id].append(msg)                                
                if len(self.paxos_promises[prop_id]) >= 3:    
                    highest_id = -1
                    chosen_val = "Tx_Default"
                    for prom in self.paxos_promises[prop_id]:
                        if prom["highest_accepted_id"] > highest_id and prom["highest_accepted_val"] is not None:    
                            highest_id = prom["highest_accepted_id"]
                            chosen_val = prom["highest_accepted_val"]                                        
                    if highest_id == -1 and hasattr(self, 'pending_tx'):
                        chosen_val = self.pending_tx                                        
                    accept_msg = {
                        "type": "PAXOS_ACCEPT",
                        "proposal_id": prop_id,
                        "proposer_id": self.node_id,                        
            ...
            prop_id = msg["proposal_id"]
            val = msg["val"]
            if self.role == "leader" and prop_id in self.paxos_accepts:
                self.paxos_accepts[prop_id] += 1
                if self.paxos_accepts[prop_id] >= 3:     
                    del self.paxos_accepts[prop_id]
    ...
    async def propose_pbft_transaction(self, tx: str):
        ...
        logging.info(f"PBFT Pre-Prepare phase initiated for sequence: {self.seq_number}")         
        self.pbft_preprepares[self.seq_number] = preprepare_msg
        await self.broadcast(preprepare_msg)

    async def process_pbft_message(self, msg: dict):
        msg_type = msg.get("type")                
        if msg_type == "PBFT_PREPREPARE":
            view = msg["view"]
            seq = msg["seq"]
            val = msg["val"]         
            digest = msg["digest"]
            ...
            payload_to_verify = msg.copy()
            payload_to_verify.pop("signature", None)
            pub_key = crypto_utils.load_public_key(signer)                        
            if not crypto_utils.verify_signature(pub_key, payload_to_verify, sig_bytes):
                logging.warning(f"PBFT Pre-Prepare signature verification failed for node {signer}!")
                return                        
            self.pbft_preprepares[seq] = msg
            self.pbft_prepares[seq] = {self.node_id}                         
            prepare_msg = {
                "type": "PBFT_PREPARE",
                "view": view,
                "seq": seq,
            ...
            sig = crypto_utils.sign_payload(self.private_key, prepare_msg)
            prepare_msg["signature"] = base64.b64encode(sig).decode('utf-8')
            await self.broadcast(prepare_msg)
            ...
            payload_to_verify = msg.copy()
            payload_to_verify.pop("signature", None)
            pub_key = crypto_utils.load_public_key(node_id)                        
            if not crypto_utils.verify_signature(pub_key, payload_to_verify, sig_bytes):
                logging.warning(f"PBFT Prepare signature verification failed for node {node_id}!")
                return                        
            if seq not in self.pbft_prepares:
                self.pbft_prepares[seq] = set()         
            self.pbft_prepares[seq].add(node_id)                        
            if len(self.pbft_prepares[seq]) >= 3 and not self.pbft_prep_certificates.get(seq, False):
                self.pbft_prep_certificates[seq] = True
                self.pbft_commits[seq] = {self.node_id}                             
                commit_msg = {
                    "type": "PBFT_COMMIT",
                    "view": self.view_number,
                    "seq": seq,                    
                ...
                sig = crypto_utils.sign_payload(self.private_key, commit_msg)
                commit_msg["signature"] = base64.b64encode(sig).decode('utf-8')
                await self.broadcast(commit_msg)        
            ...
            pub_key = crypto_utils.load_public_key(node_id)                        
            if not crypto_utils.verify_signature(pub_key, payload_to_verify, sig_bytes):
                logging.warning(f"PBFT Commit signature verification failed for node {node_id}!")
                return                        
            if seq not in self.pbft_commits: 
                self.pbft_commits[seq] = set()
            self.pbft_commits[seq].add(node_id)                        
            if len(self.pbft_commits[seq]) >= 3 and not self.pbft_committed_local.get(seq, False):
                self.pbft_committed_local[seq] = True 
                original_preprepare = self.pbft_preprepares.get(seq)
                if original_preprepare:
                    self.write_to_disk(original_preprepare["val"])
